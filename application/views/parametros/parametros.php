...
<html>
<head>
		<title>SFE Bizlinks - Par�metros</title>
		...
		<script type="text/javascript">
			...
			ncsistema=
			{
					...
					success:function(result)
						{
							...
							switch(result) {
								case '1':
									...
									Limpiar_Parametros();
									ncsistema.Listar_Parametros();
									alert('Es necesario reiniciar sesi�n y actualizar el navegador para que los cambian surtan efecto.');
									break;
								case '2':
									$('#div_MensajeValidacionEmpresa').fadeIn(0);
				...
				Guadar_Portalmultitabla:function()
				{
					...
					if (cmb_variables==7)
					{
						...
						{
							$('#div_MensajeValidacionEmpresa').fadeIn(0);
							$('#div_MensajeValidacionEmpresa').empty().append('<div style="width:10%;float:left;text-align:right"><img src="<?php echo base_url();?>application/helpers/image/ico/ncexclamacion.png"/></div><div style="margin-left:5px;font-family:Arial, Helvetica, sans-serif;font-weight:bold;font-size:12px;padding-top:3px; width:80%;float:left;text-align:left">El valor m�ximo permitido es 13%.</div>');
							setTimeout(function(){ $("#div_MensajeValidacionEmpresa").fadeOut(1500);},3000);
							return;
						}
					}
					if (txt_codid==0)
					{
						...
						{
							$('#div_MensajeValidacionEmpresa').fadeIn(0);
							$('#div_MensajeValidacionEmpresa').empty().append('<div style="width:10%;float:left;text-align:right"><img src="<?php echo base_url();?>application/helpers/image/ico/ncexclamacion.png"/></div><div style="margin-left:5px;font-family:Arial, Helvetica, sans-serif;font-weight:bold;font-size:12px;padding-top:3px; width:80%;float:left;text-align:left">No se permite generar m�s registros de este tipo por ser globales.</div>');
							setTimeout(function(){ $("#div_MensajeValidacionEmpresa").fadeOut(1500);},3000);
							return;
						}
					}else
					{
						if  (cmb_variables==8)
						{
							if (txt_valorcadena=='Configuracion_NO_Realizada')
							{
								...
								{
									$('#div_MensajeValidacionEmpresa').fadeIn(0);
									$('#div_MensajeValidacionEmpresa').empty().append('<div style="width:10%;float:left;text-align:right"><img src="<?php echo base_url();?>application/helpers/image/ico/ncexclamacion.png"/></div><div style="margin-left:5px;font-family:Arial, Helvetica, sans-serif;font-weight:bold;font-size:12px;padding-top:3px; width:80%;float:left;text-align:left">Solo se permiten valores 0(C�lculo por Valor Venta) y 1 (C�lculo por Precio)</div>');
									setTimeout(function(){ $("#div_MensajeValidacionEmpresa").fadeOut(1500);},3000);
									return;
								}else
								{
							...
							{
								$('#div_MensajeValidacionEmpresa').fadeIn(0);
								$('#div_MensajeValidacionEmpresa').empty().append('<div style="width:10%;float:left;text-align:right"><img src="<?php echo base_url();?>application/helpers/image/ico/ncexclamacion.png"/></div><div style="margin-left:5px;font-family:Arial, Helvetica, sans-serif;font-weight:bold;font-size:12px;padding-top:3px; width:80%;float:left;text-align:left">La configuraci�n se realiza por �nica vez. Comun�quese con Bizlinks!</div>');
								setTimeout(function(){ $("#div_MensajeValidacionEmpresa").fadeOut(1500);},3000);
								return;
							}
						}else
						...
						if(parseInt(result.status)==1)
						{
							$.each(result.data,function(key,rs)
							{
									...
									if (rs.grupo_id==8 )
									{
										...
										document.getElementById('div_ButtonImage').style.visibility="hidden";
										if (rs.valorentero==0)
										{ $('#div_DescripcionValorVenta_Precio').empty().append('C�lculo por Valor Venta');
										}
										else
										{$('#div_DescripcionValorVenta_Precio').empty().append('C�lculo por Precio');
										}
			...
			function cargarImagen() {
				...
				alert(files[0].size);//tama�o del archivo en KB
				return;
