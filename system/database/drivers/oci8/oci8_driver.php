...
class CI_DB_oci8_driver extends CI_DB {
	...
	public function insert_id()
	{
		...
		$sql = "SELECT UD.REFERENCED_NAME AS SEQUENCE_NAME FROM USER_DEPENDENCIES UD 
						   JOIN USER_TRIGGERS UT ON (UT.TRIGGER_NAME = UD.NAME) 
					WHERE UD.TYPE='TRIGGER' AND UD.REFERENCED_TYPE='SEQUENCE' AND ut.TABLE_NAME='SGR_EMPRESA'";//".$table."'";
		$query = $this->query($sql);
		
		$row = $query->row();
		$column1=$row->SEQUENCE_NAME;
		print_r($column1);
		return $this->query('SELECT '.$column1.'.CURRVAL AS INSERT_ID FROM DUAL')->row('INSERT_ID');
		*/
		
	}
	/*
	protected function _name_sequence($table = '')
	{
		return "SELECT UD.REFERENCED_NAME AS SEQUENCE_NAME FROM USER_DEPENDENCIES UD 
						   JOIN USER_TRIGGERS UT ON (UT.TRIGGER_NAME = UD.NAME) 
					WHERE UD.TYPE='TRIGGER' AND UD.REFERENCED_TYPE='SEQUENCE' AND ut.TABLE_NAME='".$table."'";
	}	
	*/
	public function insert_id_by_table($tablename)
	{
		$sql = "select UD.REFERENCED_NAME AS SEQUENCE_NAME FROM USER_DEPENDENCIES UD 
						   JOIN USER_TRIGGERS UT ON (UT.TRIGGER_NAME = UD.NAME) 
					WHERE UD.TYPE='TRIGGER' AND UD.REFERENCED_TYPE='SEQUENCE' AND ut.TABLE_NAME='".$tablename."'";;
		$sequencename =  $this->query($sql)->row('SEQUENCE_NAME')->SEQUENCE_NAME;
		$sql = "select ".$sequencename.".CURRVAL as insert_id  from dual";;
		return $this->query($sql)->row('insert_id');
	}
	// --------------------------------------------------------------------
